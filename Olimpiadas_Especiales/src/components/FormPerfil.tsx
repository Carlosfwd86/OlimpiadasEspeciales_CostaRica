...
const rolColor = (rol?: string) => {
    const map: Record<string, string> = { atleta:'#FF0000', entrenador:'#1e40af', tutor:'#059669', voluntario:'#7c3aed', admin:'#0f172a' };
    return rol && map[rol] ? map[rol] : '#64748b';
};

const rolBg = (rol?: string) => {
    const map: Record<string, string> = { atleta:'#FFF5F5', entrenador:'#eff6ff', tutor:'#ecfdf5', voluntario:'#f5f3ff', admin:'#f8fafc' };
    return rol && map[rol] ? map[rol] : '#f1f5f9';
};
...
function FormPerfil({ user, setRefreshUser }: FormPerfilProps): React.JSX.Element {
    ...
    const s = {
        ...
        // Header card
        headerCard: { background: '#fff', borderRadius: '20px', padding: '28px 32px', display: 'flex', alignItems: 'center', gap: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', marginBottom: '20px', flexWrap: 'wrap' as const, position: 'relative' as const },
        avatar: { width: '88px', height: '88px', borderRadius: '50%', background: color, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '2rem', fontWeight: 900, flexShrink: 0, cursor: 'pointer', position: 'relative' as const, overflow: 'hidden' },
        // Cols
        ...
        statLabel: { fontSize: '0.72rem', color: '#94a3b8', textTransform: 'uppercase' as const, letterSpacing: '0.06em', display: 'block' },
        statVal: { fontSize: '0.95rem', fontWeight: 700, color: '#0f172a' },
        // Table
        th: { fontSize: '0.72rem', fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase' as const, letterSpacing: '0.06em', padding: '6px 0', borderBottom: '1px solid #f1f5f9' },
        td: { padding: '10px 0', fontSize: '0.88rem', color: '#334155', borderBottom: '1px solid #f8fafc', verticalAlign: 'middle' },
        // Doc box
        docBox: { background: '#f8fafc', border: '1px dashed #e2e8f0', borderRadius: '12px', padding: '18px 10px', textAlign: 'center' as const, cursor: 'pointer', transition: 'all 0.2s ease' },
        // Buttons
